...
/* Time-stamp: <2010-03-07 22:56:49 cyamauch> */
...
int eggx_generatecolor( eggx_color_prms *p, double dmin, double dmax,
			double in_data, int *rt_r, int *rt_g, int *rt_b )
{
    int rt_over=0 ;
    double d_max=dmax, d_min=dmin ;
    double rt__r=0,rt__g=0,rt__b=0 ;
    double rng , data0 , ndata ;
    double _contrast , _brightness , _gamma ;
    if( d_max < d_min ){
	d_min = dmax ;
	d_max = dmin ;
	rng = d_max - d_min ;
	...
	if( 1 < p->seplevel ){
	    int num_block ;
	    double sl_block ;
	    sl_block = rng/(double)(p->seplevel) ;
...
int eggx_makecolor( int cmode, double d_min, double d_max, double data,
